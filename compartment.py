...
class MedicineCompartment:
    ...
    def refresh_inventory(self):
        """تحديث قراءة الوزن وعدد الحبات باستخدام الرياضيات الدقيقة"""
        if not self.is_open:
            # استخدام نفس الدالة التي نجحت معك
            cw = self.scale.get_weight(samples=15)
            
            # فلتر التصفير الديناميكي (إذا كان الوزن أقل من نصف حبة يعتبر صفر)
