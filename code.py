# ملف: code.py (RTOS + OOP + Fast Hardware + Serial Logging + 30s Clean Logic)final golden code
import time
import board
import digitalio
import analogio
...
# ==========================================
# 2. إعداد أدوات التحكم والطنان والشاشة
# ==========================================
...
# 🟢 تم حل مشكلة المواعيد المتقاربة لكي يرن الطنان لكل موعد 30 ثانية 🟢
def evaluate_box_alarm(box, current_h, current_m, current_s):
    ...
    for a in untaken:
        ...
        if current_mins >= alarm_mins and (current_mins - alarm_mins) < 1440:
            ...
            # نحتفظ بأقل وقت منقضي (للموعد الأحدث) لكي يرن الطنان 30 ثانية له
            if elapsed_sec < min_elapsed:
                min_elapsed = elapsed_sec
            
            # نحتفظ بأقدم موعد غير مأخوذ لتسجيله عندما يفتح المريض الصندوق
...
# ==========================================
# 5. الإقلاع ومزامنة الوقت والواي فاي 
# ==========================================
...
# 🟢 تم إلغاء اللمبة الخضراء اليتيمة وعودة النظام لطبيعته بعد 30 ثانية 🟢
def update_running_status_leds(is_alrm1, elapsed1, is_alrm2, elapsed2, current_sec):
    for b_idx in range(2):
        ...
        if boxes[b_idx].is_open: 
            ...
            # وميض أخضر في أول 30 ثانية فقط
            color = (0, 255, 0) if (current_sec % 2 == 0) else (0, 0, 0)
            for p in group: pixels[p] = color
        elif boxes[b_idx].pill_count <= 0: 
            # يعود للأحمر إذا مر الوقت والصندوق فارغ
            color = (255, 0, 0) 
            for p in group: pixels[p] = color
        else: 
            # يعود للأصفر الطبيعي إذا مر الوقت وبه حبوب
            color = (255, 180, 0) 
            for p in group: pixels[p] = color
...
# ==========================================
# 6. مهام نظام التشغيل (pyRTOS Tasks)
# ==========================================
...
def task_core_ui(self):
    ...
    while True:
        ...
        alarm_box_name = ""
        # الشاشة تعمل لمدة 30 ثانية فقط تتوافق مع الطنان
            ...
            if touch_pressed:
                ...
                if was_b1_open:
                    ...
                    if is_alrm1 and active_a1 is not None and box1.pill_count < pills_before_open_b1:
                        ...
                        # 🟢 حساب التأخير بناءً على وقت الدواء الفعلي لتسجيل التأخير بدقة 🟢
